	...
	std::unordered_set<PolyMesh::HalfedgeHandle, MeshHandleHasher>>
{
	...
	if (!f1.is_valid())
	{
		// restore original topology and bail
		m_mesh.add_face({v0, v1, v2, v3});
		m_mesh.add_face({v0, v3, v4, v5});
		return {{}, {}, {}};
	}
	...
	if (!f2.is_valid())
	{
		// f1 succeeded but f2 didn't, so undo f1, then restore originals
	...
	std::unordered_set<OpenMesh::VertexHandle, MeshHandleHasher> affectedVhs{v0, v1, v2, v3, v4, v5};
